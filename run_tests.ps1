...
try {
    ...
    docker run --rm --network digicloset-test-net -v ${PWD}/tests:/tests -e API_BASE_URL=http://backend:8000 python:3.11-slim sh -c "pip install requests python-dotenv && python /tests/unit/test_end_to_end.py"
}
finally {
    # 3. Cleanup
